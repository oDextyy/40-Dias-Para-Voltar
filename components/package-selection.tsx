...
export function PackageSelection({ quizResults, onPackageSelected }) {
  ...
  const packages = [
    {
      ...
      focus: "Acesso ao LinkedIn Premium com otimização essencial",
      recommended: false,
      color: "blue",
    },
    {
      ...
      focus: "Acesso ao LinkedIn Premium com otimização abrangente de perfil",
      recommended: true,
      color: "indigo",
    },
    {
      ...
      focus: "Acesso ao LinkedIn Premium com estratégia personalizada e técnicas avançadas",
      recommended: false,
      color: "purple",
    },
  ]
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {packages.map((pkg, index) => (
          ...
          >
            {pkg.recommended && (
              <motion.div
                className={`absolute -top-4 left-0 right-0 mx-auto w-max bg-gradient-to-r from-${pkg.color}-600 to-${pkg.color === "blue" ? "indigo" : pkg.color === "indigo" ? "purple" : "pink"}-600 text-white text-sm font-bold px-6 py-1 rounded-full shadow-lg z-10`}
                animate={{ y: [0, -3, 0] }}
            ...
            <Card
              className={`bg-white dark:bg-gray-800/90 border-gray-100 dark:border-gray-700/50 overflow-hidden h-full backdrop-blur-sm rounded-xl transition-all duration-300 ${
                selectedPackage === pkg.id ? `ring-4 ring-${pkg.color}-500/50` : ""
              } ${hoveredPackage === pkg.id ? "shadow-2xl -translate-y-2" : "shadow-xl"} ${pkg.recommended ? "mt-4" : ""}`}
            >
              <div
                className={`absolute top-0 right-0 w-64 h-64 bg-${pkg.color}-400/10 dark:bg-${pkg.color}-500/10 rounded-full blur-3xl transform translate-x-1/3 -translate-y-1/3`}
              ></div>

              <CardHeader
                className={`bg-gradient-to-r from-${pkg.color}-600 to-${pkg.color === "blue" ? "indigo" : pkg.color === "indigo" ? "purple" : "pink"}-600 text-white p-6`}
              >
              ...
              <CardContent className="p-6 relative z-10">
                <div className="mb-6">
                  <p className="text-gray-500 dark:text-gray-400 line-through text-sm">Regular: R$ {pkg.price}</p>
                  <div className="flex items-baseline">
                    <span className="text-3xl font-bold text-gray-900 dark:text-white">R$ {pkg.discountedPrice}</span>
                ...
                <div className="space-y-3 mb-6">
                  {pkg.features.map((feature, idx) => (
                    ...
                    >
                      <div className={`bg-${pkg.color}-100 dark:bg-${pkg.color}-900/30 p-1 rounded-full mt-0.5`}>
                        <CheckCircle className={`h-4 w-4 text-${pkg.color}-600 dark:text-${pkg.color}-400`} />
                      </div>
                ...
                <div
                  className={`bg-${pkg.color}-50 dark:bg-${pkg.color}-900/20 p-4 rounded-xl mb-6 border border-${pkg.color}-100 dark:border-${pkg.color}-800/30`}
                >
                  <div className="flex items-center gap-2">
                    <Star className={`h-5 w-5 text-${pkg.color}-600 dark:text-${pkg.color}-400`} />
                    <p className="text-gray-700 dark:text-gray-300 text-sm">
                      <span className={`font-bold text-${pkg.color}-700 dark:text-${pkg.color}-400`}>Foco:</span>{" "}
                      {pkg.focus}
                    </p>
                  </div>
                </div>
                ...
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Button
                    className={`w-full bg-gradient-to-r from-${pkg.color}-600 to-${pkg.color === "blue" ? "indigo" : pkg.color === "indigo" ? "purple" : "pink"}-600 hover:from-${pkg.color}-700 hover:to-${pkg.color === "blue" ? "indigo" : pkg.color === "indigo" ? "purple" : "pink"}-700 text-white py-6 rounded-xl text-lg shadow-lg`}
